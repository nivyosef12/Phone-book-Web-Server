        ...
        async def inner():
            try:
                async for db_conn in get_db():
                    ...
                    if deleted_count != cls.number_of_contacts:
                        raise Exception(f"deleted count({deleted_count}) != contacts_number({cls.number_of_contacts})")
        ...
        async def inner():
            ...
            async with httpx.AsyncClient(app=app, base_url="http://test") as client:
                ...
                while True:
                    ...
                    # assert last page contains self.number_of_contacts % limit contacts
                    assert len(response_dict) == limit or len(response_dict) == self.number_of_contacts % limit, f"fetched response does not contain the expected number of results. limit={limit}, offset={offset}, response_len={len(response_dict)}, last_page={self.number_of_contacts % limit}"

                    # update offset
                    offset += limit


                assert len(fetched_contacts) == self.number_of_contacts, f"not all contact returned. fetched_contacts({len(fetched_contacts)})={fetched_contacts}\n != {self.number_of_contacts}"
